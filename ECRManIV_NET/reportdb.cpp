...
int reportdb::inittotal(QString tablename)
{
    ...
    QString create_sql = QString("create table %1 ("
                         "Name CHARACTER(32), Cc FLOAT, Qty FLOAT, Amt FLOAT, Ret FLOAT, Disc FLOAT, Cost FLOAT, ZNo INT)").arg(tablename);
...
int reportdb::selecttable(QString tablename, unsigned int options, int ZNo, RData *data, QStandardItemModel *reportModel, int rows, char *Name, char Type)
{
    ...
    QString sql_select = QString("SELECT Name, Cc, Qty, Amt, Ret, Disc, Cost FROM %1 WHERE ZNo = :ZNo").arg(tablename);
    ...
    else
    {
        ...
        while(sql_query.next())
        {
            ...
            data->Cc = sql_query.value(1).toDouble();
            data->Qty = sql_query.value(2).toDouble();
            data->Amt = sql_query.value(3).toDouble();
            data->Ret = sql_query.value(4).toDouble();
            data->Disc = sql_query.value(5).toDouble();
            data->Cost = sql_query.value(6).toDouble();
            ...
            if(reportModel)
            {
                ...
                {
                    reportModel->setData(reportModel->index(rows_display, 0), sql_query.value(0).toString());
                ...
                else
                {
                    ...
                    {
                        reportModel->setData(reportModel->index(rows_display, 0), QString("%1(%2)").arg(QString(QLatin1String(Name))).arg(sql_query.value(0).toString()));
                    }
                    else
                    {
                ...
                qDebug() << "Cc = " << sql_query.value(1).toDouble() << "Qty = " << sql_query.value(2).toDouble()
                                << "Amt = " << sql_query.value(3).toDouble() << "Ret = " << sql_query.value(4).toDouble()
                                << "Disc = " << sql_query.value(5).toDouble() << "Cost = " << sql_query.value(6).toDouble();

                if((Type & GRANDTOTALREPORT) && (options != ZONEREPORT && options != CLERKDAILY && options != CLERKWEEK))
                {
                    reportModel->setData(reportModel->index(rows_display, 0), sql_query.value(4).toString());
                    reportModel->item(rows_display, 0)->setTextAlignment(Qt::AlignCenter);
                    ...
                    tmpdata = sql_query.value(3).toDouble() + sql_query.value(5).toDouble();
                ...
                {
#if 1
                    reportModel->setData(reportModel->index(rows_display, 1), sql_query.value(1).toString());
                    reportModel->item(rows_display, 1)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, 1)->setForeground(QBrush(QColor(255, 0, 0)));

                    reportModel->setData(reportModel->index(rows_display, 2), sql_query.value(2).toString());
                    reportModel->item(rows_display, 2)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, 2)->setForeground(QBrush(QColor(255, 0, 0)));

                    reportModel->setData(reportModel->index(rows_display, 3), sql_query.value(3).toString());
                    ...
                    reportModel->setData(reportModel->index(rows_display, 4), sql_query.value(4).toString());
                    ...
                    reportModel->setData(reportModel->index(rows_display, 5), sql_query.value(5).toString());
                    ...
                    reportModel->setData(reportModel->index(rows_display, 6), sql_query.value(6).toString());
...
{
    QString update_sql = QString("update %1 set Cc = :Cc, Qty = :Qty, Amt = :Amt, "
