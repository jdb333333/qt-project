...
void reportdb::StrToDataTime(QString timestr)
{
...
int reportdb::inittotal(QString tablename)
{
    ...
    QString create_sql = QString("create table %1 ("
                         "ID INT, IP CHARACTER(32), Name CHARACTER(32), Cc FLOAT, Qty FLOAT, Amt FLOAT, Ret FLOAT, Disc FLOAT, Cost FLOAT, XZTime DATETIME, XZFlag INT, ZNo INT)").arg(tablename);
...
int reportdb::selecttable(QString tablename, unsigned int options, int ZNo, RData *data, QStandardItemModel *reportModel, int rows, char *Name, char Type)
{
    ...
    sql_select = QString("SELECT ID, IP, Name, Cc, Qty, Amt, Ret, Disc, Cost FROM %1 WHERE XZTime = :XZTime AND ZNo = :ZNo AND XZFlag = :XZFlag").arg(tablename);
    sql_query.prepare(sql_select);
    ...
    else
    {
        ...
        while(sql_query.next())
        {
            ...
            if(reportModel)
            {
                ...
                qDebug() << "Cc = " << sql_query.value(3).toDouble() << "Qty = " << sql_query.value(4).toDouble()
                                << "Amt = " << sql_query.value(5).toDouble() << "Ret = " << sql_query.value(6).toDouble()
                                << "Disc = " << sql_query.value(7).toDouble() << "Cost = " << sql_query.value(8).toDouble();

                if((Type & GRANDTOTALREPORT) && (options != ZONEREPORT && options != CLERKDAILY && options != CLERKWEEK))
                {
                    reportModel->setData(reportModel->index(rows_display, 0), sql_query.value(6).toString());
                    reportModel->item(rows_display, 0)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, 0)->setForeground(QBrush(QColor(255, 0, 0)));
                    //tmpdata = sql_query.value(6).toDouble();
                    tmpdata = sql_query.value(8).toDouble() + sql_query.value(6).toDouble();
                    ...
                    tmpdata = sql_query.value(5).toDouble() + sql_query.value(7).toDouble();
                ...
                {
#if 1
                    reportModel->setData(reportModel->index(rows_display, 1), sql_query.value(3).toString());
                    reportModel->item(rows_display, 1)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, 1)->setForeground(QBrush(QColor(255, 0, 0)));

                    reportModel->setData(reportModel->index(rows_display, 2), sql_query.value(4).toString());
                    reportModel->item(rows_display, 2)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, 2)->setForeground(QBrush(QColor(255, 0, 0)));

                    reportModel->setData(reportModel->index(rows_display, 3), sql_query.value(5).toString());
                    ...
                    reportModel->setData(reportModel->index(rows_display, 4), sql_query.value(6).toString());
                    ...
                    reportModel->setData(reportModel->index(rows_display, 5), sql_query.value(7).toString());
                    ...
                    reportModel->setData(reportModel->index(rows_display, 6), sql_query.value(8).toString());
