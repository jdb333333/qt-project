...
int plu::DataParse(char *buf, int len)
{
    ...
    if(rows == 0)
    {
        ...
        tablehead << "Random" << "Name" << "Dept" << "Cost" << "Price" << "Inventory";
        m_varListList->push_back(tablehead);
    }

    QString Random, Name, Dept, Price, Cost, Inventory;
    ...
    tmpdata = BcdToFloat(databuf, 8, 2);
    Inventory.sprintf("%0.2f", tmpdata);

    pluinfo << Random << Name << Dept << Cost << Price << Inventory;
...
#else
        QVariant item = m_varListList->at(0).at(j);
#endif
