...
int reportdb::inittotal(QString tablename)
{
    ...
    QString create_sql = QString("create table %1 ("
                         "ID INT, IP CHARACTER(32), Name CHARACTER(32), Cc FLOAT, Qty FLOAT, Amt FLOAT, Ret FLOAT, Disc FLOAT, Cost FLOAT, ZNo INT)").arg(tablename);
...
int reportdb::selecttable(QString tablename, unsigned int options, int ZNo, RData *data, QStandardItemModel *reportModel, int rows, char *Name, char Type)
{
    ...
    QString sql_select = QString("SELECT ID, IP, Name, Cc, Qty, Amt, Ret, Disc, Cost FROM %1 WHERE ZNo = :ZNo").arg(tablename);
