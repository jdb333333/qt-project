...
int reportdb::inittotal(QString tablename)
{
    ...
    QString create_sql = QString("create table %1 ("
                         "SHOP CHARACTER(32), ID INT, IP CHARACTER(32), Name CHARACTER(32), Code CHARACTER(32), Cc FLOAT, Qty FLOAT, Amt FLOAT, Ret FLOAT, Disc FLOAT, Cost FLOAT, XZTime DATETIME, XZFlag INT, ZNo INT)").arg(tablename);
...
int reportdb::selecttable(QString tablename, unsigned int options, int ZNo, RData *data, QStandardItemModel *reportModel, int rows, char *Name, char Type)
{
    ...
    if(ClearDatetime || MaxDatetime.size() > 0){//jdb2018-07-31 已经获取最近一次时间，不需要再次获取
        Datetime = MaxDatetime;
		if(Datetime.size() == 0)
			return false;//jdb2018-07-27 没有数据返回
    }else{
        sql_select = QString("SELECT MAX(XZTime) FROM %1 WHERE ZNo = :ZNo AND XZFlag = :XZFlag").arg(tablename);
        ...
        else
        {
            ...
            MaxDatetime = Datetime;//jdb2018-07-31 保存最近一次时间
            if(MaxDatetime.size() > 0)
            	ClearDatetime = true;//jdb2018-07-27 表示已经获取最近一次时间
    ...
    else
    {
        ...
        while(sql_query.next())
        {
            ...
            if(reportModel)
            {
                ...
                qDebug() << "Code = " << sql_query.value(3).toDouble() << "Cc = " << sql_query.value(4).toDouble() << "Qty = " << sql_query.value(5).toDouble()
				...
				if(options & PLUDAILY)
				{
					qDebug() << "--->PLUDAILY";
					datarow = 1;
					reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(3).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));

					datarow++;
					reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(4).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));

					datarow++;
                    reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(5).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));

					datarow++;
                    reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(6).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));

					datarow++;
                    reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(7).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));

					datarow++;
                    reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(8).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));

					datarow++;
                    reportModel->setData(reportModel->index(rows_display, datarow), sql_query.value(9).toString());
                    reportModel->item(rows_display, datarow)->setTextAlignment(Qt::AlignCenter);
                    reportModel->item(rows_display, datarow)->setForeground(QBrush(QColor(255, 0, 0)));
				}
                else if((Type & GRANDTOTALREPORT) && (options != ZONEREPORT && options != CLERKDAILY && options != CLERKWEEK))
                {
...
int reportdb::inserttable(QString tablename, RData data)
{
    ...
    sql_query.addBindValue(ShopName);
...
int reportdb::DataParse()
{
    ...
    for(NIdx = 0; 1; NIdx++) //鑾峰彇鎶ヨ〃鍚庣紑鍚?GRAND DEPT...
    {
    ...
    int count = 4; //绗洓涓瓧鑺傚紑濮嬫槸鏈夋晥鏁版嵁
    ...
    NType = recvbuf[count++]; //鎶ヨ〃绫诲瀷
    NIdx = recvbuf[count++]; //鎶ヨ〃绱㈠紩 TIME1, TIME2, TIME3...
    ...
    if((recvbuf[3]&0xff) == 0x80)
    {
        ...
        if(NameLen > 0) //鑾峰彇鍚嶅瓧
        {
			...
			if(CodeLen > 0) //鑾峰彇缂栫爜
			{
				BcdToStr(recvbuf + count, CodeLen, ReportCode);
				count += CodeLen;

				memset(data.Code, 0, sizeof(data.Code));
				memcpy(data.Code, ReportCode, CodeLen * 2);
			}
        }
        else
        {
    ...
    qDebug() << "data.Name = " << data.Name << "data.Code = " << data.Code
