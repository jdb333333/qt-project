    ...
    ui(new Ui::ticket)
{
    ...
    QTime TimeFrom(0, 0, 0);
    DataTime.setTime(TimeFrom);

    ui->datefrom->setDateTime(DataTime);

    QTime TimeTo(23, 59, 59);
    DataTime.setTime(TimeTo);
    ui->dateto->setDateTime(DataTime);
...
void ticket::clerk_win(void)
{
    ...
    ui->clerkWidget->horizontalHeader()->setStretchLastSection(true); //设置充满表宽�?
    ...
    for(int row = 0; row < ui->clerkWidget->rowCount(); row++)
    {
        for(int column = 0; column < ui->clerkWidget->columnCount(); column++)
        {
            if(column == 0) //type
            {
                ...
                if(row == 4) //+%
                {
                    item = new QTableWidgetItem(QString("SUB+%"));
                }
                else if(row == 5)
                {
                    item = new QTableWidgetItem(QString("SUB-%"));
            ...
            if(column == 1) //name
            {
                ...
                if(row == 4) //+%
                {
                    item = new QTableWidgetItem(QString("SUBTOTAL+%"));
                }
                else if(row == 5)
                {
                    item = new QTableWidgetItem(QString("SUBTOTAL-%"));
...
void ticket::setformtype(char fmtype)
{
    ...
    else if(formtype == F_EJ)
    {
        ...
        connect(ej, SIGNAL(ejdataok(char*)), this, SLOT(ejdataok(char*)));
    }
}
...
void ticket::setiplist(QString IP, QString SHOPNAME, bool checked)
{
    ...
    if(checked)
    {
        ...
        IpStr = ui->tableWidget->item(rows, 2)->text();
    }
}
...
next:
        ...
        if(items < 3 && count + 1 == strlen(tmpdata))//不足3行补�?
        {
            m = 3 * 43 + 4;
            items = 3;
        }
...
void ticket::ej_download(void)
{
    ...
    ui->flowinfo->setStyleSheet("color:#ff6600;");
    ...
    *((WORD *)(sendbuf+2)) = 0;//发送功能码0,要收款机发送其编号数据
    ...
    size = *((WORD *)(recvbuf + 2));//获取流水顺序�?(逻辑编号)

    qDebug() << "size = " << size;

    ej->setipstr(IpStr);

    ui->ejshow->setDisabled(true);
    ui->flowinfo->setText("EJ RECEIVE DATA START!");
    ...
    for(;;)
    {
        ...
        *((WORD *)(sendbuf+2)) = size;//提取下一个流水记�?
        ...
        if(ret < 0) //send data failed
        {
            ui->flowinfo->setText("SEND DATA ERR!");
            QCoreApplication::processEvents();
            ui->ejshow->setDisabled(false);
            return;
        }

        datalen = formtcpclient->readData2(recvbuf, sizeof(recvbuf));
        if(datalen < 0) //receive data failed
        {
            ui->flowinfo->setText("RECEIVE DATA ERR!");
            QCoreApplication::processEvents();
            ui->ejshow->setDisabled(false);
            return;
        }
        ...
        ui->flowinfo->setText(QString("EJ RECEIVE DATA ITEM%1!").arg(itemcount++));
        QCoreApplication::processEvents();
    }
    ui->flowinfo->setText("EJ RECEIVE DATA FINISHED!");
    QCoreApplication::processEvents();
...
void ticket::ejdataok(char *databuf)
{
    ui->EJ_TXT->append(databuf);
}

void ticket::on_ejshow_clicked()
{
    ej->setipstr(IpStr);
    QStandardItemModel *reportModel = new QStandardItemModel(this);
    QStringList titles;
    titles << "Name" << "Description" << "Number" << "Sales" << "Data";
...
#endif
#if 0
    for(int column = 0; column < reportModel->columnCount(); column++)
    {
    ...
    QTime TimeTo(23, 59, 59);
    ui->dateto->dateTime().setTime(TimeTo);

    memset(recvbuf, 0, sizeof(recvbuf));
    ui->EJ_TXT->clear();
    ej->DataCreate(recvbuf, reportModel, ui->datefrom->dateTime(), ui->dateto->dateTime());
#ifdef ReceiptLOG
    //ui->EJ_TXT->append(recvbuf);
#endif
}
...
void ticket::on_tableWidget_clicked(const QModelIndex &index)
{
    IpStr = ui->tableWidget->item(index.row(),2)->text();
