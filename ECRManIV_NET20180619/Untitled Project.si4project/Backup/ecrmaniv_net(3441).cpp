    ...
    ui(new Ui::ECRManIV_NET)
{
    ...
    ui->tableWidget_2->setSelectionBehavior(QAbstractItemView::SelectRows); //整行选中的方�?
    ...
    ui->tableWidget_2->verticalHeader()->setVisible(false);   //隐藏列表�?
    ...
    //ui->tableWidget_2->setStyleSheet("selection-background-color:lightblue;"); //设置选中背景�?
    ...
    if(rows > 0)
    {
        ...
        for(int count = 0; count < rows; count++)
        {
            if(count == rowIndex)
            {
                QString ipstr = ui->tableWidget_2->item(count,1)->text();//取出字符�?
                qDebug() << "-------->ipstr = " << ipstr;
                myticket->setiplist(ipstr, true);
            }
            else
            {
                QString ipstr = ui->tableWidget_2->item(count,1)->text();//取出字符�?
                qDebug() << "-------->ipstr = " << ipstr;
                myticket->setiplist(ipstr, false);
            }
        }
    }
    ...
    if(rows > 0)
    {
        ...
        for(int count = 0; count < rows; count++)
        {
            if(count == rowIndex)
            {
                QString ipstr = ui->tableWidget_2->item(count,1)->text();//取出字符�?
                qDebug() << "-------->ipstr = " << ipstr;
                myplu->setiplist(ipstr, true);
            }
            else
            {
                QString ipstr = ui->tableWidget_2->item(count,1)->text();//取出字符�?
                qDebug() << "-------->ipstr = " << ipstr;
                myplu->setiplist(ipstr, false);
            }
        }
    }
    ...
    if(rows > 0)
    {
        ...
        for(int count = 0; count < rows; count++)
        {
            if(count == rowIndex)
            {
                QString ipstr = ui->tableWidget_2->item(count,1)->text();//取出字符�?
                qDebug() << "-------->ipstr = " << ipstr;
                mydept->setiplist(ipstr, true);
            }
            else
            {
                QString ipstr = ui->tableWidget_2->item(count,1)->text();//取出字符�?
                qDebug() << "-------->ipstr = " << ipstr;
                mydept->setiplist(ipstr, false);
            }
        }
    }
