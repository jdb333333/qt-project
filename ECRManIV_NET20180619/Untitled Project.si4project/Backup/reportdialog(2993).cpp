...
void reportDialog::tcpconnectcash()
{
	...
	recvflag = true;//jdb2018-06-20 ¡¨Ω”∑˛ŒÒ∆˜«∞£¨Ω” ’±Í÷æ÷√Œª
    ...
    for(;IpIndex < IpIndexMax; IpIndex++)
    {
        if(ui->tableWidget->item(IpIndex, 0)->checkState() == Qt::Checked)
        {
            ...
            ShopName = ui->tableWidget->item(IpIndex, 2)->text(); //jdb2018-07-12 ªÒ»°µÍ∆Ã√˚
...
void reportDialog::tcpstate(int type)
{
    ...
    if(tcpstat == TCPRCVOK)
    {
        if(udflag == UPDATE_DATA) //ÂÖàËé∑Âèñ‰ø°ÊÅ?
        {
            //
        }
...
void reportDialog::recvdataok(int sizelen)
{
    ...
    {//jdb2018-06-20 pluΩ” ’ ˝æ›±®±Ì
        ...
        else if((recvbuf[4 + 2] & 0xff) == 'O'  && (recvbuf[5 + 2] & 0xff) == 'K')
        {
			...
			if(XZModle == 0x01)//jdb2018-06-20 x±®±Ì÷ªΩ” ’“ª¥Œ ˝æ›
            {
            	IpIndex++;
				tcpconnectcash();
			}
    ...
    if(sizelen == 6 && XZModle == 0x02 && recvbuf[0 + 2] == 'R' && recvbuf[4 + 2] == 'O' && recvbuf[5 + 2] == 'K') //¥Ú”°OK
    {
    	udflag = UPDATE_DATA;
        qDebug() << "z print ok";
		memset(recvbuf, 0, sizeof(recvbuf));
    ...
    if(udflag == UPDATE_DATA)
    {
        ...
        {
            cashid = recvbuf[7];
            udflag = DOWNLOAD_DATA; //ÂºÄÂßã‰∏ãËΩΩÊúâÊïàÊï∞Êç?
