...
int reportDialog::recvdataok(int sizelen)
{
	...
	//PLU
    if((recvbuf[0] & 0xff) == 0x5A && (recvbuf[2] & 0xff) == DOWNLOAD_DATA && (recvbuf[3] & 0xff) == PLUREPORT)
    {
        if((recvbuf[4] & 0xff) != 'O'  && (recvbuf[5] & 0xff) != 'K')
        {
            if(recvflag)
            {
                itemindex--;
        ...
        else if((recvbuf[4] & 0xff) == 'O'  && (recvbuf[5] & 0xff) == 'K')
        {
            itemindex++;
    ...
    if(m_sizelen == 6 && XZModle == 0x02 && recvbuf[0] == 'R' && recvbuf[4] == 'O' && recvbuf[5] == 'K') //??ӡOK
    {
        udflag = UPDATE_DATA;
        qDebug() << "z print ok";
		switch(zbtnnum)
    ...
    {
        MyDb->SetDataType(0x0);
        MyDb->datadeal(recvbuf, m_sizelen);
    }
    else if(XZModle == 0x02)
    {
        MyDb->SetDataType(DataReportType);
    ...
    if(IpIndex < IpIndexMax)
    {
        ...
        *((unsigned short *)(sendbuf[0] + 2)) = 100;
		...
		else
		{
			...
			if(ret < 0)
			{
				ui->info->setText(QString("send data to %1 failed.").arg(IpStr));
				QCoreApplication::processEvents();//避免界面冻结
        		IpIndex++;
				goto nextconnect;
			}
