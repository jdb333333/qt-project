    ...
    ui(new Ui::reportDialog)
{
    ...
    ui->tableWidget->verticalHeader()->setVisible(false);   //闅愯棌鍒楄〃澶?
    ...
    ui->tableWidget->setSelectionBehavior(QAbstractItemView::SelectRows); //鏁磋閫変腑鐨勬柟寮?
    ui->tableWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);   //绂佹淇敼
    //ui->tableWidget->setFrameShape(QFrame::NoFrame); //璁剧疆鏃犺竟妗?
    //ui->tableWidget->horizontalHeader()->setStyleSheet("QHeaderView::section{background:skyblue;}");//璁剧疆琛ㄥご鑳屾櫙鑹?
    //ui->tableWidget->horizontalHeader()->setHighlightSections(false);//鐐瑰嚮琛ㄦ椂涓嶅琛ㄥご琛屽厜浜紙鑾峰彇鐒︾偣锛?
...
void reportDialog::tcpconnectcash()
{
	...
	recvflag = true;//jdb2018-06-20 连接服务器前，接收标志置位
    ...
    for(;IpIndex < IpIndexMax; IpIndex++)
    {
        if(ui->tableWidget->item(IpIndex, 0)->checkState() == Qt::Checked)
        {
            ...
            IpStr = ui->tableWidget->item(IpIndex,1)->text();//鍙栧嚭瀛楃涓?
            ShopName = ui->tableWidget->item(IpIndex, 2)->text(); //jdb2018-07-12 获取店铺名
            ui->info->setText(QString("start connection to %1").arg(IpStr));
            StripToZno(IpStr);
            QCoreApplication::processEvents();//閬垮厤鐣岄潰鍐荤粨
            qDebug() << "--->set new ip";
...
void reportDialog::dbstate(int type)
{
    ...
    if(dbstat == WRITEDBOK)
    {
        ...
        {
#if 0
            //ui->cleardbreport->setEnabled(true); //jdb2018-07-12 取消清数据库功能
            emit unconnecthost();
#else
            ...
            if(XZModle == 0x02) //涓篫鎶ヨ〃锛屼笅杞藉畬鍚庢樉绀哄搴旂殑鎶ヨ〃
            {
            #if 0
                switch(zbtnnum)
                {
...
void reportDialog::recvdataok(int sizelen)
{
    ...
    {//jdb2018-06-20 plu接收数据报表
        ...
        else if((recvbuf[4 + 2] & 0xff) == 'O'  && (recvbuf[5 + 2] & 0xff) == 'K')
        {
			...
			if(XZModle == 0x01)//jdb2018-06-20 x报表只接收一次数据
            {
            	IpIndex++;
				tcpconnectcash();
			}
    ...
    if(sizelen == 6 && XZModle == 0x02 && recvbuf[0 + 2] == 'R' && recvbuf[4 + 2] == 'O' && recvbuf[5 + 2] == 'K') //打印OK
    {
    	udflag = UPDATE_DATA;
        qDebug() << "z print ok";
		memset(recvbuf, 0, sizeof(recvbuf));
    ...
    if(udflag == UPDATE_DATA)
    {
        ...
        {
            cashid = recvbuf[7];
            udflag = DOWNLOAD_DATA; //寮�濮嬩笅杞芥湁鏁堟暟鎹?
...
int reportDialog::downloaddata(unsigned char datatype)
{
    ...
    datatype = GRANDTOTALREPORT | DEPARTMENTREPORT | TENDERREPORT | DRAWERREPORT |
        ...
        //ui->cleardbreport->setEnabled(false); //jdb2018-07-12 取消清数据库功能
...
{
    bool focus = ui->tableWidget->isItemSelected(ui->tableWidget->currentItem()); // 鍒ゆ柇鏄惁閫変腑涓�琛?
    if(focus)
    {
        int row1 = ui->tableWidget->currentItem()->row(); // 褰撳墠閫変腑琛?
        IpStr = ui->tableWidget->item(row1,1)->text();//鍙栧嚭瀛楃涓?
        StripToZno(IpStr);
        emit setserverip(IpStr);
